    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <section style={{ background: 'linear-gradient(160deg, #1a2744 0%, #1e3460 100%)', padding: '80px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(77,184,232,0.2)', border: '1px solid rgba(77,184,232,0.4)', borderRadius: '50px', padding: '6px 16px', marginBottom: '24px' }}>
            <span style={{ color: '#7dcef0', fontSize: '13px', fontWeight: '600' }}>🔧 MSP — Datto RMM — Kaseya 365 — UseSecure</span>
          </div>
          <h1 style={{ fontSize: 'clamp(28px, 4vw, 52px)', fontWeight: '800', color: '#fff', lineHeight: '1.15', marginBottom: '20px' }}>
            ...
            <span style={{ background: 'linear-gradient(135deg, #2aa8de, #4db8e8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>préventive pour TPE/PME</span>
          </h1>
      ...
      <section style={{ padding: '80px 24px', background: '#f0f8ff' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: 'clamp(26px, 3vw, 40px)', fontWeight: '800', color: '#1a2744', marginBottom: '12px' }}>Ce qui est inclus</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
            {services.map((s, i) => (
              <div key={i} style={{ background: '#fff', borderRadius: '16px', padding: '28px', border: '1px solid rgba(77,184,232,0.12)', boxShadow: '0 4px 15px rgba(77,184,232,0.06)' }}>
                ...
                <h3 style={{ fontSize: '17px', fontWeight: '700', color: '#1a2744', marginBottom: '8px' }}>{s.title}</h3>
      ...
      <section style={{ padding: '80px 24px', background: '#fff' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: 'clamp(26px, 3vw, 40px)', fontWeight: '800', color: '#1a2744', marginBottom: '12px' }}>Nos packs maintenance</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }} className="grid-2-1f">
            {packs.map((p, i) => (
              <div key={i} style={{ background: p.color ? 'linear-gradient(135deg, #1a2744, #1e3460)' : '#f0f8ff', borderRadius: '20px', padding: '40px 32px', border: p.color ? '2px solid #4db8e8' : '2px solid rgba(77,184,232,0.2)', position: 'relative' }}>
      ...
      <section style={{ padding: '80px 24px', background: 'linear-gradient(135deg, #1a2744, #1e3460)', textAlign: 'center' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          ...
          <p style={{ color: '#7dcef0', fontSize: '18px', marginBottom: '36px' }}>Gratuit et sans engagement — réponse sous 24h</p>
