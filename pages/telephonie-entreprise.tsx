...
export default function Telephonie() {
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* HERO */}
      <section style={{ background: 'linear-gradient(160deg, #1a2744 0%, #1e3460 100%)', padding: '80px 24px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px', alignItems: 'center' }} className="hero-grid-1f">
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(77,184,232,0.2)', border: '1px solid rgba(77,184,232,0.4)', borderRadius: '50px', padding: '6px 16px', marginBottom: '24px' }}>
              <span style={{ color: '#7dcef0', fontSize: '13px', fontWeight: '600' }}>📞 Revendeur officiel Yealink — France entière</span>
            </div>
            <h1 style={{ fontSize: 'clamp(28px, 3.5vw, 48px)', fontWeight: '800', color: '#fff', lineHeight: '1.15', marginBottom: '20px' }}>
              Téléphonie IP<br />
              <span style={{ background: 'linear-gradient(135deg, #2aa8de, #4db8e8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Yealink pour entreprises</span>
            </h1>
            <p style={{ color: '#7dcef0', fontSize: '17px', lineHeight: '1.7', marginBottom: '32px' }}>
            ...
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              <a href="/#contact" style={{ background: 'linear-gradient(135deg, #4db8e8, #2aa8de)', color: '#fff', padding: '14px 28px', borderRadius: '10px', textDecoration: 'none', fontSize: '15px', fontWeight: '700', boxShadow: '0 8px 30px rgba(77,184,232,0.5)' }}>🚀 Demander un devis</a>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            {[{ val: '10€', label: 'par ligne/mois leasing' }, { val: '5€', label: 'ligne au compteur' }, { val: '14€', label: 'ligne illimitée' }, { val: '36 mois', label: 'leasing sans apport' }].map(s => (
              <div key={s.val} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(77,184,232,0.25)', borderRadius: '12px', padding: '20px', textAlign: 'center' }}>
                <div style={{ fontSize: '28px', fontWeight: '800', color: '#2aa8de', marginBottom: '4px' }}>{s.val}</div>
      ...
      <section style={{ padding: '80px 24px', background: '#f0f8ff' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: 'clamp(26px, 3vw, 40px)', fontWeight: '800', color: '#1a2744', marginBottom: '12px' }}>Gamme Yealink</h2>
            <p style={{ color: '#6b21a8', fontSize: '17px' }}>Séries T7x, T8x fixes — W59R & W78P sans fil</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
            {gammeYealink.map((t, i) => (
              <div key={i} style={{ background: '#fff', borderRadius: '16px', padding: '24px', border: '1px solid rgba(77,184,232,0.15)', boxShadow: '0 4px 15px rgba(77,184,232,0.06)' }}>
                <div style={{ background: 'linear-gradient(135deg, #4db8e8, #2aa8de)', color: '#fff', fontSize: '11px', fontWeight: '700', padding: '3px 10px', borderRadius: '50px', display: 'inline-block', marginBottom: '10px' }}>{t.usage}</div>
                <h3 style={{ fontSize: '18px', fontWeight: '800', color: '#1a2744', marginBottom: '6px' }}>{t.serie}</h3>
                <p style={{ color: '#4a6080', fontSize: '13px', lineHeight: '1.6', marginBottom: '12px' }}>{t.desc}</p>
                <div style={{ color: '#4db8e8', fontWeight: '700', fontSize: '14px' }}>{t.prix}</div>
              </div>
            ))}
          </div>
          <p style={{ textAlign: 'center', color: '#4a6080', fontSize: '13px', marginTop: '24px' }}>* Leasing 36 mois Grenke/Investitel — sans apport — charges 100% déductibles</p>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 24px', background: '#fff' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: 'clamp(26px, 3vw, 40px)', fontWeight: '800', color: '#1a2744', marginBottom: '12px' }}>Abonnements ligne</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }} className="grid-2-1f">
            {abonnements.map((a, i) => (
              <div key={i} style={{ background: i === 1 ? 'linear-gradient(135deg, #1a2744, #1e3460)' : '#f0f8ff', borderRadius: '20px', padding: '36px', border: i === 1 ? '2px solid #4db8e8' : '1px solid rgba(77,184,232,0.2)' }}>
                <div style={{ fontSize: '32px', marginBottom: '16px' }}>{a.icon}</div>
                <h3 style={{ fontSize: '22px', fontWeight: '800', color: i === 1 ? '#fff' : '#1a2744', marginBottom: '8px' }}>{a.type}</h3>
                <div style={{ fontSize: '28px', fontWeight: '800', color: '#2aa8de', marginBottom: '12px' }}>{a.prix}</div>
                <p style={{ color: i === 1 ? '#7dcef0' : '#4a6080', fontSize: '14px', lineHeight: '1.6' }}>{a.desc}</p>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: '80px 24px', background: '#f0f8ff' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: 'clamp(26px, 3vw, 40px)', fontWeight: '800', color: '#1a2744', marginBottom: '12px' }}>Fonctionnalités incluses</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '12px' }}>
            {fonctionnalites.map((f, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', background: '#fff', padding: '16px 20px', borderRadius: '10px', border: '1px solid rgba(77,184,232,0.12)' }}>
                <span style={{ color: '#4db8e8', fontWeight: '700', fontSize: '16px', flexShrink: 0 }}>✓</span>
      ...
      <section style={{ padding: '80px 24px', background: 'linear-gradient(135deg, #1a2744, #1e3460)', textAlign: 'center' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          ...
          <p style={{ color: '#7dcef0', fontSize: '18px', marginBottom: '36px' }}>Réponse sous 24h — installation partout en France</p>
