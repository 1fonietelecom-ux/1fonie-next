    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <section style={{ background: 'linear-gradient(160deg, #1e0b3e 0%, #2d1458 100%)', padding: '80px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(124,58,237,0.2)', border: '1px solid rgba(124,58,237,0.4)', borderRadius: '50px', padding: '6px 16px', marginBottom: '24px' }}>
            <span style={{ color: '#c084fc', fontSize: '13px', fontWeight: '600' }}>🛡️ MSP certifié — SOC Kaseya 24/7/365 — Datto RMM — UseSecure</span>
          </div>
          <h1 style={{ fontSize: 'clamp(28px, 4vw, 52px)', fontWeight: '800', color: '#fff', lineHeight: '1.15', marginBottom: '20px' }}>
            Cybersécurité pour<br />
            <span style={{ background: 'linear-gradient(135deg, #a855f7, #7c3aed)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>TPE & PME</span>
          </h1>
          ...
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="/#contact" style={{ background: 'linear-gradient(135deg, #7c3aed, #a855f7)', color: '#fff', padding: '14px 28px', borderRadius: '10px', textDecoration: 'none', fontSize: '15px', fontWeight: '700', boxShadow: '0 8px 30px rgba(124,58,237,0.5)' }}>🚀 Audit gratuit de votre sécurité</a>
      ...
      <section style={{ padding: '80px 24px', background: '#fff' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: 'clamp(26px, 3vw, 40px)', fontWeight: '800', color: '#1e0b3e', marginBottom: '12px' }}>Les menaces que nous bloquons</h2>
            <p style={{ color: '#6b21a8', fontSize: '17px' }}>En France, 1 PME sur 2 a subi une cyberattaque en 2023</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
            {menaces.map((m, i) => (
              <div key={i} style={{ background: '#faf5ff', borderRadius: '16px', padding: '28px', border: '1px solid rgba(124,58,237,0.12)' }}>
                <div style={{ fontSize: '32px', marginBottom: '12px' }}>{m.icon}</div>
                <h3 style={{ fontSize: '17px', fontWeight: '700', color: '#1e0b3e', marginBottom: '8px' }}>{m.title}</h3>
                <p style={{ color: '#6b7280', fontSize: '13px', lineHeight: '1.6' }}>{m.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '80px 24px', background: '#faf5ff' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <h2 style={{ fontSize: 'clamp(26px, 3vw, 40px)', fontWeight: '800', color: '#1e0b3e', marginBottom: '12px' }}>Nos packs cybersécurité</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }} className="grid-2-1f">
            {packs.map((p, i) => (
              <div key={i} style={{ background: p.color ? 'linear-gradient(135deg, #1e0b3e, #2d1458)' : '#fff', borderRadius: '20px', padding: '40px 32px', border: p.color ? '2px solid #7c3aed' : '2px solid rgba(124,58,237,0.2)', position: 'relative' }}>
      ...
      <section style={{ padding: '80px 24px', background: 'linear-gradient(135deg, #1e0b3e, #2d1458)', textAlign: 'center' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          ...
          <p style={{ color: '#c084fc', fontSize: '18px', marginBottom: '36px' }}>Nous analysons votre niveau de protection actuel — sans engagement</p>
